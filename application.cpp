...
GLuint loadShader(const char *vertexfilename, const char *fragmentfilename) {
  ...
  // glShaderSource
  string a = getTextFile(vertexfilename);
  const char * abc = a.c_str();
  string b = getTextFile(fragmentfilename);
  const char * xyz = b.c_str();
  
  glShaderSource(vertex_shader_id, 1, &abc, NULL);
  glShaderSource(fragment_shader_id, 1, &xyz, NULL);
  // glCompileShader
